```html //DesignSystem version 2023
<fieldset class="form-options form-options--radio">
  <legend class="form-options-legend">Legend <span class="field-required">*</span></legend>
  <label class="form-options-label" for="radio1">
    <input class="form-options-field form-options-field--radio" id="radio1" type="radio" name="radio-group" value="radio1" required>
    <span class="form-options-description">Radio 1</span>
  </label>
  <label class="form-options-label" for="radio2">
    <input class="form-options-field form-options-field--radio" id="radio2" type="radio" name="radio-group" value="radio2" required>
    <span class="form-options-description">Radio 2</span>
  </label>
  <label class="form-options-label" for="radio3">
    <input class="form-options-field form-options-field--radio" id="radio3" type="radio" name="radio-group" value="radio3" required>
    <span class="form-options-description">Radio 3</span>
  </label>
</fieldset>
```